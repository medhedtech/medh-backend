...
class ZoomService {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        qs.stringify({
          ...
          scope:
            "meeting:write:meeting meeting:write:meeting:admin meeting:read:meeting meeting:read:meeting:admin user:read:user:admin user:read:user user:write user:write:admin",
        }),
        {
          headers: {
      ...
      if (
        error.response?.data?.reason === "Invalid client_id or client_secret"
      ) {
        console.error(
  ...
  async addMeetingRegistrant(meetingId, registrantData) {
    try {
      ...
      console.error(
        "Error adding meeting registrant:",
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async updateRegistrantStatus(meetingId, registrantData) {
    try {
      ...
      console.error(
        "Error updating registrant status:",
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  generateSignature(meetingNumber, role = 0) {
    try {
      ...
      const msg = Buffer.from(
        this.clientId + meetingNumber + timestamp + role,
      ).toString("base64");
      const hash = crypto
        ...
        .digest("base64");
      return Buffer.from(
        `${this.clientId}.${meetingNumber}.${timestamp}.${role}.${hash}`,
      ).toString("base64");
    } catch (error) {
  ...
  async getMeetingRecordings(meetingId) {
    try {
      ...
      console.error(
        "Error getting meeting recordings:",
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async getRecordingSettings(meetingId) {
    try {
      const token = await this.getAccessToken();
      const response = await axios.get(
        `${this.baseUrl}/meetings/${meetingId}/recordings/settings`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  async updateRecordingSettings(meetingId, settings) {
    try {
      ...
      console.error(
        "Error updating recording settings:",
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async getMeetingAudioTranscript(meetingId) {
    try {
      const token = await this.getAccessToken();
      const response = await axios.get(
        `${this.baseUrl}/meetings/${meetingId}/recordings/transcript`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      console.error(
        "Error getting meeting transcript:",
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async createClassroomMeeting(userId = "me", classroomData) {
    try {
      ...
      console.error(
        "Error creating classroom meeting:",
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async getRegistrants(meetingId, options = {}) {
    try {
      ...
      console.error(
        "Error getting meeting registrants:",
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async getInvitation(meetingId) {
    try {
      ...
      console.error(
        "Error getting meeting invitation:",
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async createInstructorUser(userData) {
    try {
      ...
      console.error(
        "Error creating instructor user:",
        error.response?.data || error.message,
      );
      throw error;
    }
