  ...
  try {
    ...
    if (type === "subscription") {
      ...
      htmlContent = generatePdfContentForSubscription({
        ...
        start_date:
          data.enrollment_date?.toLocaleDateString() ||
          new Date().toLocaleDateString(),
        payment_id:
          data.payment_details?.payment_id ||
          data.payment_details?.payment_order_id,
        payment_date:
          data.payment_details?.payment_date?.toLocaleDateString() ||
          new Date().toLocaleDateString(),
    ...
    const folderPath =
      type === "subscription" ? "subscriptions" : "enrollments";
  ...
  try {
    ...
    if (payment_type === "course") {
      ...
      const subscription = new Subscription({
        ...
        start_date,
        end_date: new Date(
          start_date.setMonth(start_date.getMonth() + duration_months),
        ),
        payment_status: "completed",
        payment_id: paymentResponse?.razorpay_payment_id || "",
...
export const getStudentPayments = async (req, res) => {
  try {
    ...
    const [
      ...
      Subscription.find({ user_id: student_id }).lean(),
    ]);
    ...
    const allPaymentsForStats = [
      ...allFormattedEnrollments,
      ...allFormattedSubscriptions,
    ].sort((a, b) => new Date(b.enrollmentDate) - new Date(a.enrollmentDate));
    ...
    const activeSubscriptions = allFormattedSubscriptions.filter(
    ...
    res.status(200).json({
      ...
      statistics: {
        ...
        lastPayment: {
          ...
          formattedDate: lastPayment
            ? new Date(lastPayment.enrollmentDate).toLocaleDateString("en-US", {
                year: "numeric",
                month: "short",
                day: "numeric",
              })
...
export const getPaymentStats = async (req, res) => {
  try {
    ...
    const totalRevenueAmount =
      (totalRevenue[0][0]?.total || 0) + (totalRevenue[1][0]?.total || 0);
    ...
    const studentSpendingList = Array.from(studentSpendingMap.values()).sort(
...
export const generateReceiptForExistingPayment = async (req, res) => {
  try {
    ...
    if (receiptUrl) {
      ...
      // Send email with receipt
      const email =
        payment_type === "course" ? payment.student_email : payment.user_email;
      if (email) {
        await sendReceiptEmail(
          email,
          receiptUrl,
