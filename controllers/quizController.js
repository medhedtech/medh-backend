...
exports.uploadQuiz = async (req, res) => {
  try {
    ...
    worksheet.eachRow({ includeEmpty: false, skipHeader: true }, (row, rowNumber) => {
      if (rowNumber > 1) { // Skip header row
        const question = {
          ...
          difficultyLevel: row.getCell(9).value?.toString().trim() || 'Medium'
        };

        // Validate question
        if (!question.question || question.options.length < 2 || !question.correctAnswer) {
          rejectedItems.push({ rowNumber, ...question, error: 'Missing required fields' });
        } else if (!question.options.includes(question.correctAnswer)) {
          rejectedItems.push({ rowNumber, ...question, error: 'Correct answer must match one of the options' });
        } else {
    ...
    res.status(201).json({
      status: 'success',
      message: quiz.isNew ? 'Quiz created successfully' : 'Quiz updated successfully',
      data: quiz,
      rejectedItems
    });
  } catch (error) {
...
exports.exportQuizToExcel = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=${encodeURIComponent(quiz.quiz_title)}.xlsx`);
    res.send(buffer);
  } catch (error) {
