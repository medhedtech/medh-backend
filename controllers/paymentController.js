...
import * as paymentProcessor from "./payment-controller.js";
...
export const getComprehensivePaymentHistory = async (req, res) => {
  try {
    ...
    const EnrolledCourse = (await import("../models/enrolled-courses-model.js"))
      .default;
        ...
        paymentHistory.push({
          ...
          emi_details:
            enrollment.payment_type === "emi"
              ? {
                  total_installments:
                    enrollment.emiDetails?.numberOfInstallments,
                  ...
                  overdue_installments:
                    enrollment.emiDetails?.schedule?.filter((s) => {
                      return (
                        s.status === "pending" &&
    ...
    res.status(200).json({
      ...
      data: {
        ...
        summary: {
          ...
          total_amount: paymentHistory.reduce(
            (sum, p) => sum + (p.amount || 0),
            0,
          ),
          total_courses: enrollments.length,
          emi_enrollments: enrollments.filter((e) => e.payment_type === "emi")
            .length,
        },
      },
    });
...
const calculatePaymentAnalytics = (paymentHistory, enrollments) => {
  ...
  const regularTotal = regularPayments.reduce(
    (sum, p) => sum + (p.amount || 0),
    0,
  );
  ...
  const emiMetrics = {
    ...
    active_emi_enrollments: enrollments.filter(
      (e) =>
        e.payment_type === "emi" &&
        ["active", "pending"].includes(e.payment_status),
    ).length,
    ...
    overdue_amount: enrollments.reduce((total, enrollment) => {
      if (
        ...
        const now = new Date();
        const overdueInstallments = enrollment.emiDetails.schedule.filter(
  ...
  return {
    financial_overview: {
      ...
      average_transaction_amount:
        paymentHistory.length > 0 ? totalSpent / paymentHistory.length : 0,
      largest_transaction: Math.max(
    ...
    transaction_analytics: {
      ...
      success_rate:
        paymentHistory.length > 0
          ? (successfulTransactions / paymentHistory.length) * 100
          : 100,
    },
    ...
    financial_health: {
      ...
      emi_default_risk:
        emiMetrics.overdue_amount > 0
          ? "high"
          : emiMetrics.total_pending_amount > emiMetrics.total_emi_amount * 0.3
            ? "medium"
            : "low",
...
const calculatePaymentFrequency = (payments) => {
  if (payments.length < 2)
    return {
      average_days_between_payments: 0,
...
const getNextPaymentDue = (enrollments) => {
  ...
  enrollments.forEach((enrollment) => {
    if (enrollment.payment_type === "emi" && enrollment.emiDetails?.schedule) {
      ...
      if (pendingInstallments.length > 0) {
        const nextInstallment = pendingInstallments.sort(
          (a, b) => new Date(a.dueDate) - new Date(b.dueDate),
        )[0];
        if (
          !nextPaymentDate ||
          new Date(nextInstallment.dueDate) < new Date(nextPaymentDate)
        ) {
  ...
  return nextPaymentDate
    ? { date: nextPaymentDate, amount: nextPaymentAmount }
    : null;
};
