...
export const getFormByFormId = catchAsync(async (req, res, next) => {
  const form = await UniversalForm.findOne({ 
    ...
    is_deleted: false 
  }).select('form_id form_type status submitted_at processed_at contact_info.full_name');
...
export const updateForm = catchAsync(async (req, res, next) => {
  const { status, priority, assigned_to, internal_note, follow_up_required, follow_up_date } = req.body;
  ...
  if (follow_up_required !== undefined) form.follow_up_required = follow_up_required;
...
async function sendFormConfirmationEmail(form) {
  ...
  const emailData = {
    ...
    data: {
      ...
      form_id: form.form_id,
      message: form.message.substring(0, 200) + (form.message.length > 200 ? '...' : '')
    }
  };
...
function getStatusMessage(status) {
  const messages = {
    ...
    rejected: 'Thank you for your submission. Unfortunately, we cannot proceed at this time.',
...
function convertToCSV(forms) {
  ...
  const headers = [
    'Form ID', 'Form Type', 'Status', 'Priority', 'Name', 'Email', 'Phone',
    'Company', 'Message', 'Submitted At', 'Processed At', 'Assigned To'
  ];
