...
try {
  ...
  const { default: DailyRotateFile } = await import(
  ...
  logger.error(
    "Failed to initialize winston-daily-rotate-file, falling back to standard file transport",
    err,
  );
}
      ...
      format: format.combine(
        ...
        format.printf(({ level, message, timestamp, metadata }) => {
          ...
          if (filteredMeta && Object.keys(filteredMeta).length > 0) {
            metaString = util.inspect(filteredMeta, {
              colors: true,
              depth: null,
            });
          }
