...
router.post("/meetings", async (req, res) => {
  try {
    ...
    if (error.response?.status === 401) {
      res
        .status(401)
        .json({ message: "Unauthorized. Check Zoom API credentials." });
  ...
  try {
    ...
    for (const recording of recordings) {
      ...
      const processedRecording =
        await zoomService.processRecording(recordingData);
...
router.patch("/users/:userId", async (req, res) => {
  try {
    const user = await zoomService.updateUserProfile(
      req.params.userId,
      req.body,
    );
    res.json(user);
...
router.patch("/meetings/:meetingId/recordings/settings", async (req, res) => {
  try {
    const { meetingId } = req.params;
    const settings = await zoomService.updateRecordingSettings(
      meetingId,
      req.body,
    );
    res.json({
...
router.post("/classroom/meetings", async (req, res) => {
  try {
    ...
    const meetingDetails = await zoomService.createClassroomMeeting(
      userId,
      req.body,
    );
    res.status(201).json({
...
router.post("/meetings/:meetingId/registrants", async (req, res) => {
  try {
    const { meetingId } = req.params;
    const registrant = await zoomService.addMeetingRegistrant(
      meetingId,
      req.body,
    );
    res.status(201).json({
...
router.put("/meetings/:meetingId/registrants/status", async (req, res) => {
  try {
    const { meetingId } = req.params;
    const result = await zoomService.updateRegistrantStatus(
      meetingId,
      req.body,
    );
    res.json({
...
router.get("/meetings/:meetingId/join/:registrantId", async (req, res) => {
  try {
    ...
    const joinLink = await zoomService.generateJoinLink(
      meetingId,
      registrantId,
    );
    res.json({
