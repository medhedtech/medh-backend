...
const connectDB = async () => {
  try {
    await mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/medh');
      ...
      if (week.lessons && week.lessons.length > 0) {
        ...
        week.lessons.forEach((lesson, lessonIndex) => {
          ...
          if (needsVideoFields) {
            ...
            lesson.video_url = 'https://medhdocuments.s3.ap-south-1.amazonaws.com/videos/placeholder.mp4';
            lesson.duration = '30 minutes';
            
            // If we can infer from the title that it should have specific content
            if (lesson.title.includes('3454')) {
              lesson.video_url = 'https://medhdocuments.s3.ap-south-1.amazonaws.com/videos/1749140025780-fxzbr5.mp4';
              lesson.duration = '33 minutes';
            }
            
            updatedCount++;
          } else {
            console.log(`  ✅ Lesson already has video fields: ${lesson.title}`);
          }
        });
      }
    });
