...
const backupDatabase = async () => {
  ...
  try {
    ...
    const backupPath = path.join(config.backup.directory, `backup-${config.backup.timestamp}`);
    const mongoConfig = getMongoConfig();
    
    if (!fs.existsSync(config.backup.directory)) {
      fs.mkdirSync(config.backup.directory, { recursive: true });
      console.log(`📁 Created backup directory: ${config.backup.directory}`.gray);
    }
    ...
    console.log(`📄 Restore instructions saved to: ${backupPath}/RESTORE_INSTRUCTIONS.md`.blue);
...
const backupSpecificCollections = (backupPath) => {
  return new Promise((resolve, reject) => {
    ...
    collections.forEach(collection => {
      ...
      exec(command, (error, stdout, stderr) => {
        if (error) {
          console.error(`❌ Failed to backup ${collection}:`.red, error.message);
          hasErrors = true;
        } else {
...
const createBackupMetadata = async (backupPath) => {
  ...
  const metadata = {
    ...
    migration: {
      purpose: 'Pre-migration backup before legacy to new course types migration',
