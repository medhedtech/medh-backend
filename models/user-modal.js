...
const userSchema = new Schema(
  {
    ...
    twitter_link: {
      ...
      validate: {
        validator: function (v) {
          return (
            !v || /^https?:\/\/(?:www\.)?(?:twitter\.com|x\.com)\/.+/i.test(v)
          );
        },
...
userSchema.pre("save", function (next) {
  ...
  const isProfileUpdated = profileFields.some((field) =>
...
userSchema.methods.endSession = function (sessionId) {
  ...
  if (session) {
    ...
    this.statistics.engagement.total_session_time =
      (this.statistics.engagement.total_session_time || 0) + session.duration;
...
userSchema.statics.findOnlineUsers = function () {
  return this.find({ is_online: true }).select(
    "full_name username user_image last_seen",
  );
};
...
userSchema.methods.getLoginStats = function () {
  return {
    ...
    login_frequency: this.login_analytics?.login_frequency || {
      daily: 0,
      weekly: 0,
      monthly: 0,
    },
    device_stats: this.login_analytics?.device_stats || {
      mobile: 0,
      tablet: 0,
      desktop: 0,
    },
    ...
    average_session_duration:
      this.login_analytics?.average_session_duration || 0,
...
userSchema.methods.getLoginPattern = function () {
  if (!this.login_history || this.login_history.length < 2) {
    return {
      ...
      description: "Not enough login data",
    };
  }

  const frequency = this.login_analytics?.login_frequency;
  if (!frequency)
    return { pattern: "unknown", description: "No frequency data available" };
...
userSchema.methods.getSecurityScore = function () {
  ...
  const rapidLogins = recentLogins.filter((login, index) => {
    ...
    const timeDiff =
      new Date(recentLogins[index - 1].timestamp) - new Date(login.timestamp);
