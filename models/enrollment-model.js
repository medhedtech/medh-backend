...
enrollmentSchema.methods.updateStatus = async function(newStatus) {
  if (!["active", "completed", "dropped", "suspended", "expired"].includes(newStatus)) {
...
enrollmentSchema.methods.updateProgress = async function(progressData) {
  ...
  if (!this.progress[type].completed.includes(itemId)) {
    ...
    const completedItems = this.progress.lessons.completed.length + 
...
enrollmentSchema.methods.calculateTotalItems = async function() {
  ...
  return course.lessons.length + course.assignments.length + course.quizzes.length;
};
...
enrollmentSchema.methods.updatePaymentStatus = async function(status, details = {}) {
  if (!["pending", "completed", "failed", "refunded", "partial"].includes(status)) {
