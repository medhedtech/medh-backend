...
export const validateCreateBooking = [
  ...
  body('timeSlot')
    ...
    .custom((value) => {
      ...
      if (bookingDate > maxBookingTime) {
        throw new Error('Demo cannot be scheduled more than 90 days in advance');
      }
      ...
      if (hour < 9 || hour >= 18) {
        throw new Error('Demos can only be scheduled between 9 AM and 6 PM UTC');
      }
  ...
  body('timezone')
    ...
    .custom((value) => {
      if (value && !moment.tz.names().includes(value)) {
        throw new Error('Invalid timezone. Please provide a valid IANA timezone identifier');
      }
      return true;
    }),
  ...
  body('demoType')
    .optional()
    .isIn(['course_demo', 'consultation', 'product_walkthrough', 'general_inquiry'])
    .withMessage('Demo type must be one of: course_demo, consultation, product_walkthrough, general_inquiry'),
  ...
  body('source')
    ...
    .withMessage('Source must be one of: website, social_media, referral, advertisement, other'),
...
export const validateGetUserBookings = [
  ...
  query('status')
    .optional()
    .isIn(['pending', 'confirmed', 'cancelled', 'rescheduled', 'completed', 'no-show'])
    .withMessage('Status must be one of: pending, confirmed, cancelled, rescheduled, completed, no-show'),
...
export const validateUpdateBooking = [
  ...
  body('action')
    ...
    .withMessage('Action must be one of: cancel, reschedule, confirm, complete'),
...
export const checkDuplicateBooking = async (req, res, next) => {
  try {
    ...
    const DemoBooking = (await import('../models/demo-booking.model.js')).default;
