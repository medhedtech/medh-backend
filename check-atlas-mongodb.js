...
async function checkAtlasMongoDB() {
  try {
    ...
    console.log('✅ MongoDB Atlas MedhDB से connect हो गया');
    
    const { Batch } = await import('./models/course-model.js');
    
    console.log('\n🔍 MongoDB Atlas में data check कर रहा हूँ...\n');
    
    // 1. Database info
    console.log('📋 Step 1: Database info:');
    console.log(`Database name: ${mongoose.connection.name}`);
    console.log(`Database host: ${mongoose.connection.host}`);
    
    // 2. Collections check करें
    console.log('\n📋 Step 2: Collections check कर रहा हूँ...');
    const collections = await mongoose.connection.db.listCollections().toArray();
    
    console.log('Available collections in MedhDB:');
    ...
    if (rawBatches.length > 0) {
      console.log('\nRaw batch data (first batch):');
      console.log(JSON.stringify(rawBatches[0], null, 2));
    }
    
    await mongoose.disconnect();
    console.log('\n✅ MongoDB Atlas data check पूरा हुआ!');
