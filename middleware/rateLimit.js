...
const createRateLimitHandler = (limitType) => {
  return (req, res) => {
    ...
    res.status(429).json({
      success: false,
      status: 'error',
      message: RATE_LIMITS[limitType].message || 'Too many requests, please try again later',
...
// Create specific limiters for export
export const loginLimiter = getRateLimiter('LOGIN');
export const registerLimiter = getRateLimiter('REGISTER');
export const passwordResetLimiter = getRateLimiter('PASSWORD_RESET');
